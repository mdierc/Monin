...
* Peter Kruis			  06/01/2024 Retrieve VLF, different for SQL Server 2016
**********************************************************************************************************************************************************************/
...
SET @TempTablename_01 = '##tb_VLF_temp_'
                        + REPLACE(CONVERT(VARCHAR(128), NEWID()), '-', '');

SET @SQLVersion = SUBSTRING(
                              @@VERSION,
                              CHARINDEX(@@VERSION, 'Microsoft SQL Server ')
                              + 22,
                              4
                           );
IF @SQLVersion = '2008'
BEGIN
   SET @CmdStr = 'CREATE TABLE [' + @TempTablename_01
                 + '] 
           (FileID VARCHAR(3),
...
SET @TempTablename_02 = '##tb_VLF_temp_'
                        + REPLACE(CONVERT(VARCHAR(128), NEWID()), '-', '');
SET @CmdStr = 'CREATE TABLE [' + @TempTablename_02
...
SET @CmdStr = 'SELECT ''' + @MachineName
              + ''' AS [Server Name], name AS [Database Name],
       vlf_count as [VLF Count]
  FROM ['     + @TempTablename_02
              + ']
  
 ORDER BY [Database Name], vlf_count DESC;
